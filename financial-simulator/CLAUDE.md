...
npm test                # sync + engine + montecarlo tests — fast, no browser
...
The pure-logic modules (`engine.js`, `montecarlo.js`, `payroll.js`, `recurrence.js`, `format.js`, `seeds.js`, `store.js`) have no React dependency and are `import`ed directly in tests — no browser or stubbing needed.
...
- `tests/engine.test.mjs` — unit tests for `simulateWeekly` (`src/engine.js`) and `payrollOf`/`bonusOf` (`src/payroll.js`) against small deterministic scenarios: employer-match formula, bonus withholding, card-interest-only-on-carried-balance, highest-APR-first debt rollover, and the account-cap sweep respecting the "redirect into investing" setting.
- `tests/montecarlo.test.mjs` — unit tests for `runMonteCarlo`.
- `tests/e2e.test.mjs` — Playwright tests against the real served page: the front-page link, the help panel (closed by default, follows the active tab), `localStorage` persistence across reload, the one-time warning toast when storage writes fail, the redirect toggle, and a regression test that edits income and triggers every tab's chart tooltip (a past bug — a missing import in a shared component — only surfaced once a Tooltip actually rendered).

There is no CI workflow in this repository currently; `.github/workflows/sync-to-site.yml` runs on pushes to `main` and rsyncs the repo contents into the `liamsalter11.github.io` site repo (excluding `.git`, `.github`, `node_modules`, `test-results`, `playwright-report`) — it does not run tests.
